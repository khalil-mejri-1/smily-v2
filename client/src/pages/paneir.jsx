...
  FiX,
  FiCheckCircle,
  FiClipboard, // New icon for orders history
} from "react-icons/fi";
...
const ORDERS_STORAGE_KEY = "pastOrders"; // Key for storing past orders
...
  const [orderSuccessMessageVisible, setOrderSuccessMessageVisible] = useState(false);
...
      const updatedStoredCart = JSON.parse(localStorage.getItem(CART_STORAGE_KEY)) || [];
      setCartItems(updatedStoredCart);
      // Update past orders on storage change too
      const updatedStoredOrders = JSON.parse(localStorage.getItem(ORDERS_STORAGE_KEY)) || [];
      setPastOrders(updatedStoredOrders);
    };
...
    setTimeout(() => {
      const newItems = JSON.parse(localStorage.getItem(CART_STORAGE_KEY) || "[]").filter(
...
  // دالة لحذف الطلب من التخزين المحلي والـ State
  const handleDeleteOrder = (orderId) => {
    const updatedOrders = pastOrders.filter(order => order.id !== orderId);
    setPastOrders(updatedOrders);
    localStorage.setItem(ORDERS_STORAGE_KEY, JSON.stringify(updatedOrders));
    toast.success("Order removed successfully from history.");
    
    // إغلاق النافذة إذا لم يتبق طلبات
    if (updatedOrders.length === 0) {
      setShowOrdersHistory(false);
    }
  };

  // دالة إرسال الطلب وحفظه في Local Storage
...
      // --- حفظ الطلب في Local Storage وتحديث الحالة ---
...
          {/* الزر العائم لتاريخ الطلبات */}
          {pastOrders.length > 0 && (
            <button className="orders-history-btn" onClick={() => setShowOrdersHistory(true)}>
              <FiClipboard />
              <span className="order-count">{pastOrders.length}</span>
            </button>
          )}
...
          {/* نافذة عرض تاريخ الطلبات */}
          {showOrdersHistory && (
            <div className="modal-overlay">
              <div className="modal-content orders-history-modal">
                <div className="modal-header">
                  <h2><FiClipboard /> Your Order History ({pastOrders.length})</h2>
                  <button className="close-btn" onClick={() => setShowOrdersHistory(false)}><FiX /></button>
                </div>
...
                        <h3>Order # {pastOrders.length - index}</h3>
                        <p className="order-date">Date: {formatDate(order.orderDate)}</p>
<br />
                        <p className="order-date">ID: <span className="order-id-text-main">{order.id}</span></p>
                      </div>
...
                              <p className="item-title-name"><strong>{item.title}</strong></p>
                            



  <p className="item-info-text">Size: {item.size || "Standard"}</p>
{/*                               <p className="item-info-text">Product ID: <span className="order-id-text">{item.productId}</span></p> */}
                            </div>
                            <div className="item-quantity-price">
                              <span className="item-quantity">x{item.quantity}</span>
                              <span className="item-price-detail">{(parseFloat(item.price) * item.quantity).toFixed(2)} DT</span>
                            </div>
                          </div>
<br />
  </>
...
                        <strong>Total Price: {order.totalPrice} DT</strong>
                        {/* 🛑 زر الحذف الجديد 🛑 */}
                        <button 
