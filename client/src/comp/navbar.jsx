    ...
    <>
      ...
      <aside className={`cart-sidebar_panier ${isOpen ? "open" : ""}`}>
        ...
        {cartItems.length > 0 ? (
          <>
                  ...
                  <div className="cart-item-actions">
                    <div className="cart-item-price">
                      ...
                      <span className="original-price">
                        {(parseFloat(item.originalPrice) * item.quantity).toFixed(2)}{" "}DT
                      </span>
                    </div>
                    <button
            ...
            <div className="cart-footer">
              <div className="subtotal">
                <span>Subtotal</span>
                <div>
                  <span style={{ color: "#f7d900", textShadow: " 0 0 8px rgba(247, 217, 0, 0.3)", fontWeight: "700", fontSize: "20px" }}>
                    {subtotal} DT
                  </span>
                  <br />
                  <span style={{ color: " #718096", fontWeight: "500", fontSize: "18px", textDecoration: "line-through", marginLeft: "7px" }}>
          ...
          <div className="cart-empty" style={{ position: "relative", textAlign: "center" }}>
            <p>Your shopping cart is empty.</p> <br />
            <Link to="/product" className="empty-cart-button" onClick={onClose}> Continue Shopping </Link>
          </div>
        )}
      </aside>
    </>
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const handleCategoryClick = (category) => {
    if (category.subCategories && category.subCategories.length > 0) {
      ...
      window.location.href = `/product?category=${encodeURIComponent(category.name)}`;
      onClose();
    }
  };
  ...
  return (
    <>
      ...
      <aside className={`sidebar ${isOpen ? "open" : ""}`}>
        <div className={`sidebar-panel ${!activeSubcategories ? "panel-active" : "panel-hidden-left"}`}>
          <div className="sidebar-header">
            <h3>Categories</h3>
            <button onClick={onClose} className="header-btn"><FaTimes /></button>
          </div>
          <ul className="category-list">
            {categoryData.map((category) => (
              <li key={category.name}>
                <a onClick={() => handleCategoryClick(category)}>
                  <span > <img src={category.icon} className="category-icon"/></span> {category.name}
                  {category.subCategories && category.subCategories.length > 0 && <span className="arrow">{">"}</span>}
                </a>
              </li>
            ))}
          </ul>
        </div>
        <div className={`sidebar-panel ${activeSubcategories ? "panel-active" : "panel-hidden-right"}`}>
          <div className="sidebar-header">
            <button onClick={handleBackClick} className="header-btn"><FaArrowLeft /></button>
            <h3>{subCategoryTitle}</h3>
          </div>
          <ul className="category-list">
            {activeSubcategories && (
              <li key="all-subcategories">
                <Link to={`/product?mainCategory=${encodeURIComponent(subCategoryTitle)}&subcats=${activeSubcategories.map(encodeURIComponent).join(",")}`} onClick={onClose}>
                  All {subCategoryTitle} <span className="arrow">{">"}</span>
                </Link>
              </li>
            )}
            {activeSubcategories && activeSubcategories.map((subcategory) => (
              <li key={subcategory}>
                <Link to={`/product?category=${encodeURIComponent(subcategory)}`} onClick={onClose}>
...
const SearchUI = ({
  query, setQuery, isSearchLoading, isDropdownVisible, setDropdownVisible,
  results, hasResults, handleSearchSubmit, handleLinkClick, navigate,
}) => {
      ...
      {isDropdownVisible && query && (
        <div className="search-suggestions">
          {isSearchLoading ? (
                  ...
                  {results.categories.map((cat) => (
                    <li key={cat}>
                      <Link 
                        ...
                        onClick={(e) => handleSafeNavigate(e, `/product?category=${cat}`)}
              ...
              {results.products.map((item) => (
                <li key={item._id}>
                  <Link 
                    to="/ProductDetail" 
                    state={{ productData: item }} 
                    onClick={(e) => handleSafeNavigate(e, "/ProductDetail", { productData: item })}
                    // ✅ --- FIX: Stop the mousedown event to prevent race condition
                    onMouseDown={(e) => e.stopPropagation()}
                  >
            ...
            <div className="no-results"><p>No results found for "{query}"</p></div>
          )}
          {hasResults && !isSearchLoading && (
            <div className="show-more-container">
              <Link 
                ...
                onClick={(e) => handleSafeNavigate(e, `/product?recherche=${query}`)}
                // ✅ --- FIX: Stop the mousedown event to prevent race condition
                onMouseDown={(e) => e.stopPropagation()}
              >
...
const Navbar = () => {
  ...
  useEffect(() => {
    if (query.trim() === "") {
      ...
      setDropdownVisible(false); setSearchLoading(false); return;
    }
    setDropdownVisible(true); setSearchLoading(true);
    const delayDebounceFn = setTimeout(async () => {
      try {
        const response = await axios.get(`https://smily-la3j.vercel.app/search/suggestions?q=${query}`);
  ...
  return (
    <>
      ...
      <nav className="navbar">
        <div className="navbar_mini">
          <div className="navbar-left">
            <button className="nav-button shop-button" onClick={() => setSidebarOpen(true)}>
          ...
          <div className="navbar-right">
            <button className="nav-button cart-button" onClick={() => setCartOpen(true)}>
              <FaShoppingCart />
      ...
      <footer className="mobile-bottom-nav">
        <button className="mobile-nav-button" onClick={() => setSidebarOpen(true)}>
          <FaBars /> <span>Shop</span>
        </button>
        <button className="mobile-nav-button" onClick={() => setMobileSearchOpen(true)}>
